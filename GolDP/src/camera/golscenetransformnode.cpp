...
{
	m_unk0x18 = new GolDPCoordSys[m_capacity];
	if (m_unk0x18 == NULL) {
...
GolICoordSys* GolSceneTransformNode::VTable0x18(LegoU32 p_index) const
{
...
void GolSceneTransformNode::FUN_10014a60(const GolMatrix4& p_m1, const GolMatrix4& p_m2, const GolMatrix4& p_m3)
{
	...
	if (m_unk0x14 != NULL) {
		...
		for (; obj < end; obj++) {
			GolDPCoordSys* parent = static_cast<GolDPCoordSys*>(obj->m_parent);
			if (parent == NULL) {
				GolMath::FUN_1002f3a0(obj->m_localTransform, p_m1, &obj->m_unk0x50);
			}
			else {
				GolMath::FUN_1002f3a0(obj->m_localTransform, parent->m_unk0x50, &obj->m_unk0x50);
			}
		}
	}
}
...
void GolSceneTransformNode::VTable0x20(const GolMatrix4& p_m)
{
	if (m_unk0x14 == NULL) {
		for (LegoU32 i = 0; i < m_capacity; i++) {
			GolDPCoordSys* obj = &m_unk0x18[i];
			GolDPCoordSys* parent = static_cast<GolDPCoordSys*>(obj->m_parent);
			if (parent != NULL) {
				GolMath::FUN_1002f450(obj->m_localTransform, parent->m_unk0x90, &obj->m_unk0x90);
			}
			else {
				GolMath::FUN_1002f450(obj->m_localTransform, p_m, &obj->m_unk0x90);
			}
		}
		return;
	}
	...
	for (LegoU32 i = 0; i < m_capacity; i++) {
		...
		if (m_unk0x14->VTable0x00(i)) {
			GolQuat rotation;
			GolMath::FUN_1002f720(obj->m_localTransform, &rotation);

			GolVec3 position;
			position.m_x = obj->m_localTransform.m_m[3][0];
			position.m_y = obj->m_localTransform.m_m[3][1];
			position.m_z = obj->m_localTransform.m_m[3][2];

			m_unk0x14->VTable0x04(i, rotation, position, parentMatrix, &obj->m_unk0x90);
		}
		else {
			GolMath::FUN_1002f450(obj->m_localTransform, parentMatrix, &obj->m_unk0x90);
		}
	}
}
...
{
	GolDPCoordSys* obj = &m_unk0x18[p_index];
...
LegoU32 GolSceneTransformNode::VTable0x1c(const GolICoordSys& p_obj) const
{
	return &static_cast<const GolDPCoordSys&>(p_obj) - m_unk0x18;
}
